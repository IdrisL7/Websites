    ...
    if (!candidateHooks) {
      try {
        ...
        if (tierId === "pro" || tierId === "concierge") {
          ...
          // Prefer user-provided LinkedIn URL for accurate slug; fall back to domain guess
          const linkedinSlug = (() => {
            if (linkedinUrlRaw) {
              try {
                const liUrl = new URL(linkedinUrlRaw.startsWith("http") ? linkedinUrlRaw : `https://${linkedinUrlRaw}`);
                const match = liUrl.pathname.match(/\/company\/([^/?#]+)/);
                if (match?.[1]) return match[1];
              } catch {}
            }
            return companyDomain ? companyDomain.split(".")[0] : undefined;
          })();
