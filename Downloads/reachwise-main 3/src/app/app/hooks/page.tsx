...
export default function HooksPage() {
  ...
  async function addToWatchlist() {
    ...
    try {
      await fetch("/api/watchlist", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          companyUrl: url || `https://${companyDomain}`,
    ...
    try {
      const res = await fetch("/api/generate-hooks", {
        ...
        body: JSON.stringify({
          ...
          linkedinUrl: userTier !== "starter" && linkedinUrl.trim() ? linkedinUrl.trim() : undefined,
        }),
      });
  ...
  return (
    <div>
      ...
      {!loading && hooks.length === 0 && !error && !upgradePrompt && (
        <EmptyState
      ...
      {lowSignal && hooks.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
          <span>Tip: Try a more specific source for stronger hooks.</span>
          {linkedinSlug && (
            <button
          ...
          {firstPartyUrls.slice(0, 2).map((d, i) => (
            <button
              key={i}
              onClick={() => runWithUrl(d.url)}
              className="text-zinc-400 hover:text-zinc-200 underline underline-offset-2 transition-colors truncate max-w-[180px]"
            >
              {d.title || d.url}
            </button>
          ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 justify-between">
                ...
                <button
                  ...
                  disabled={watchState === "watching" || watchState === "watched"}
                ...
                >
                  {watchState === "watched" ? "✓ Watching" : watchState === "watching" ? "Saving…" : "Watch"}
                </button>
              ...
              <p className="text-[11px] text-zinc-600">
                Each hook is grounded in a real signal — Tier A = first-party or reputable source, B = secondary, C = low confidence. Hover Tier badges for details.
              </p>
