    ...
    >
      ...
      {hook.source_snippet && (
        <div className="mb-3 rounded-lg border border-amber-500/20 bg-amber-500/[0.04] px-3 py-2.5">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-amber-500/70 mb-1">Proof</p>
          <p className="text-xs text-zinc-300 leading-relaxed">{hook.source_snippet}</p>
          {hook.source_url && (
            <a
              href={hook.source_url}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-1.5 flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors truncate"
            >
              {hook.source_url.includes("linkedin.com") && (
                <svg className="h-3 w-3 text-[#0077b5] shrink-0" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
              )}
              <span className="truncate">{hook.source_title || hook.source_url}</span>
            </a>
          )}
        </div>
      )}
